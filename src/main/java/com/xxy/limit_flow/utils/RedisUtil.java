package com.xxy.limit_flow.utils;
...
public class RedisUtil {
    ...
    // 增加计数器的值
    //boundValueOps方法返回一个BoundValueOperations对象，
    // 提供了一组操作来处理与特定键（key）相关联的字符串值。
    //设置值：使用set方法设置指定键的字符串值。
    //获取值：使用get方法获取指定键的字符串值。
    //删除键：使用delete方法删除指定键及其对应的值。
    //操作过期时间：使用expire方法设置键的过期时间。
    //自增或自减：使用increment方法对存储为整数的字符串值进行原子递增或递减操作。
    //其他字符串操作：例如拼接字符串、获取子串等。
    public Long increment(String key, long delta) {
        return jsonRedisTemplate.boundValueOps(key).increment(delta);
    }
    ...
    // 重置键的值和过期时间
    public void reset(String key, int value, long expire) {
        jsonRedisTemplate.boundValueOps(key).set(value);
        jsonRedisTemplate.expire(key, expire, TimeUnit.SECONDS);
    }
